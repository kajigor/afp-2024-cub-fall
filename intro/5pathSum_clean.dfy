  ...
  // we want to return true only if there exists a path in a tree that has the given sum
{
  ...
  assert leftPath ==> exists p: seq<TreeNode> ::
        isPath(p, root.left) && pathSum(p) == targetSum-root.val &&
        isPath([root] + p, root) && pathSum([root] + p) == targetSum;
  assert rightPath ==> exists p: seq<TreeNode> ::
        isPath(p, root.right) && pathSum(p) == targetSum-root.val &&
        isPath([root] + p, root) && pathSum([root] + p) == targetSum;
